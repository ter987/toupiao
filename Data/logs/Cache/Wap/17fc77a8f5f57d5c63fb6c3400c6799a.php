...
<div id="outer">
    ...
    <dl id="tab">
	<dd class="current"  style=" background:url(images/nav_bg1.png) no-repeat #44b6c9;">参赛选手</dd>
    <dd style="background:url(images/nav_bg2.png) no-repeat #f59f46;">活动介绍</dd>
    ...
    <button type="submit" title="搜索" class="button"><span><span>搜索</span></span></button>
		...
		<ul class="cc">
			<?php if(is_array($vote_item)): $i = 0; $__LIST__ = $vote_item;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$li): $mod = ($i % 2 );++$i;?><li><a href="<?php echo C('site_url');?>/index.php?g=Wap&m=Vote&a=show&token=<?php echo ($token); ?>&id=<?php echo ($li["id"]); ?>&wecha_id=<?php echo ($wecha_id); ?>&tid=<?php echo ($id); ?>"><img src="<?php echo ($li["startpicurl"]); ?>" style="width:100%; height:117px;"></a>
		...
		<script type="text/javascript">
            ...
            function loadData(){
				...
				if ($(document).height() <= totalheight) { 
                    ...
                    $.get("index.php?g=Wap&m=Vote&a=add_item&id=<?php echo ($vote["id"]); ?>&key=<?php echo ($search); ?>&page="+newpage,
                        function(data){
                            if(data == ''){
            ...
            function loadRank(){
				...
				var newpage = parseInt(page);
                    ...
                    $.get("index.php?g=Wap&m=Vote&a=add_rank&id=<?php echo ($vote["id"]); ?>&key=<?php echo ($search); ?>&page="+newpage,
                        function(data){
                            if(data == ''){
...
var url = "<?php echo C('site_url'); echo U('Vote/index',array('token'=>$token,'id'=>$id));?>";
...
shareWeibo();
     ...
     <div style="width:100%;height:50px;position:fixed;bottom:0;background:#fd564a;font-family:'Microsoft YaHei';color:#fff;z-index:1000;">
        <dl style="width:24.2%;height:48px;padding-top:2px;text-align:center;float:left;border-left:1px solid #fe685e;border-right:1px solid #fa4034;"><a style="color:#fff;" href="<?php echo U('Vote/index',array('token'=>$token,'id'=>$id));?>"><dt><img width="28" height="28" src="images/shouye.png" /></dt><dd>首 页</dd></a></dl>
         ...
         <dl style="width:24.2%;height:48px;padding-top:2px;text-align:center;float:left;border-left:1px solid #fe685e;border-right:1px solid #fa4034;"><a style="color:#fff;" href="<?php echo U('Vote/share',array('token'=>$token,'id'=>$id,'g'=>'user'));?>"><dt><img width="28" height="28" src="images/paihang.png" /></dt><dd>排行榜</dd></a></dl>
