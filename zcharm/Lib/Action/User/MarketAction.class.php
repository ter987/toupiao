...
class MarketAction extends UserAction
{
    ...
    {
        $flash = M("Market_flash")->where(array('token' => $this->token, 'cid' => $this->_cid))->select();
        ...
        public function flashadd()
    {
        ...
        if (IS_POST) { 
            ...
            if ($data->create()) {
                if ($id) {
                    if ($data->save()) {
                        $this->success('修改成功', U('Market/flash',array('token' => session('token'))));
                    } else {
                        $this->error('操作失败');
                    }
                } else {
                    if ($data->add()) {
                        $this->success('添加成功', U('Market/flash',array('token' => session('token'))));
                    } else {
                        $this->error('操作失败');
                    }
                }
            ...
            $flash = M("Market_flash")->where(array('token' => $this->token, 'cid' => $this->_cid, 'id' => $id))->find();
    ...
    public function flashdel()
    {
        ...
        $where['cid']=$this->_cid;
        if(D("Market_flash")->where($where)->delete()){
            $this->success('操作成功',U('Market/flash',array('token' => session('token'))));
        }else{
            $this->error('操作失败',U('Market/flash',array('token' => session('token'))));
        }
    }
}