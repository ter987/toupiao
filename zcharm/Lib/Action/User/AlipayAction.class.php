...
//		}
	...
	public function vip_post (){
		...
		if ($needFee<$moneyBalance||$needFee==$moneyBalance){
			...
			//
			$this->success('处理成功，请退出重新登陆才会生效',U('User/Index/info'));
		}else{
	...
	public function setconfig(){
		...
		$alipay_config['cacert']    = getcwd().'\\31cms\\Lib\\ORG\\Alipay\\cacert.pem';
			...
			if( $this->_get('trade_status') == 'TRADE_FINISHED' ||  $this->_get('trade_status') == 'TRADE_SUCCESS') {
				...
				if($indent!=false){
					...
					if($back!=false){
						...
						$this->error('充值失败,请在线客服,为您处理',U('User/Index/info'));
					}
				}else{
					$this->error('订单不存在',U('User/Index/info'));
				...
				if($indent!=false){
					...
					if($back!=false){
						...
						if ($needFee<$moneyBalance){
							...
							//
							$this->success('充值成功并购买成功',U('User/Index/info'));
						}else{
							$this->success('充值成功但您的余额不足',U('User/Index/info'));
						}
					}else{
						$this->error('充值失败,请在线客服,为您处理',U('User/Index/info'));
					}
