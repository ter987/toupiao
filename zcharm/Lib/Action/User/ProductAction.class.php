...
class ProductAction extends UserAction{
	...
	public function orderInfo(){
		...
		if (IS_POST){
			...
			if ($thisOrder['paytype']=='weixin'&&$thisOrder['transactionid']){
				$this->success('修改成功,正在同步发货状态到微信�?,U('Product/deliveryNotify',array('token'=>session('token'),'orderid'=>$thisOrder['orderid'],'wecha_id'=>$thisOrder['wecha_id'],'transactionid'=>$thisOrder['transactionid'],'id'=>$thisOrder['id'])));
			}else {
	...
	function deliveryNotify(){
		...
		if (!$json->errmsg){
			...
			$this->error('获取access_token发生错误：错误代�?.$json->errcode.',微信返回错误信息�?.$json->errmsg);
		}
	...
	function api_notice_increment($url, $data){
		...
		if ($errorno) {
			...
			if ($js['errcode']=='0'){
				...
				$this->error('发生错误：错误代�?.$js['errcode'].',微信返回错误信息�?.$js['errmsg']);
			}
		}
	}
